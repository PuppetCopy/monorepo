...
export const $PortfolioEditorDrawer = (config: IPortfolioEditorDrawer) => component((
  ...
  const hasDraft = skipRepeatsWith(draft => draft.matchRuleList.length > 0 || draft.depositTokenList.length > 0, combineState({ matchRuleList, depositTokenList }))
    ...
    switchMap(params => {
      ...
      return fadeIn($card2(style({ border: `1px solid ${colorAlpha(pallete.foreground, .20)}`, padding: '18px 0', borderBottom: 'none', borderRadius: '20px 20px 0 0' }))(
        $column(spacing.default)(
          ...
          $column(spacing.default, style({ overflow: 'auto', maxHeight: '35vh', padding: `0 ${screenUtils.isDesktopScreen ? '24px' : '12px'}` }))(
            ...portfolioRouteList.map(route => {
              return $column(style({ paddingLeft: '16px' }))(
                $row(spacing.big, style({ padding: '6px 0' }))(
                  $RouteDepositEditor({
