import { Behavior, combineArray, combineState, replayLatest } from "aelea/core"
...
import { combineState, getMappedValue, parseFixed, parseReadableNumber, promiseState, PromiseStatus, readableTokenAmount, readableTokenAmountLabel, switchMap } from "@puppet/middleware/utils"
        ...
        $IntermediateConnectButton({
          ...
          $$display: map(wallet => {
            ...
            return $ApproveSpend({
              ...
              token: change.token,
              amount: map(params => params.action === DepositEditorAction.DEPOSIT ? params.amount : 0n, combineState({ amount, action })),
              walletClient: wallet,
