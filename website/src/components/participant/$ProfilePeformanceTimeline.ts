...
export const $ProfilePeformanceTimeline = (config: IProfilePeformanceTimeline) => component((
  ...
  const debouncedState = debounce(40, combineState({ selectedCollateralTokenList, pricefeedMapQuery, activityTimeframe, matchRuleList }))
    ...
    $column(style({ width: '100%', padding: 0, height: '200px', placeContent: 'center' }))(
      ...
      $IntermediatePromise({
        ...
        $$done: map(params => {
          ...
          if (positionCount === 0) {
            return $row(spacing.defaultTiny, style({ textAlign: 'center', placeSelf: 'center', }))(
