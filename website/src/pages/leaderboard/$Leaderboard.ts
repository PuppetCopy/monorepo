...
import { getPositionListTimelinePerformance, IPerformanceTimelineTick } from "../../components/trade/$ProfilePerformanceGraph.js"
...
export const $Leaderboard = (config: ILeaderboard) => component((
  ...
  return [
    $column(spacing.default, style({ paddingTop: '36px' }))(

      $card2(style({ padding: "0", gap: 0 }))(
        $responsiveFlex(spacing.big, style({ padding: '26px', placeContent: 'space-between', alignItems: 'center' }))(
        ...
        $IntermediatePromise({
          ...
          $$done: map(params => {
            ...
            }, combineState({ activityTimeframe, paging, sortBy, isLong, collateralTokenList: selectedCollateralTokenList, account }))
