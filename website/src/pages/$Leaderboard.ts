...
import { $card2, $labeledDivider } from '../common/elements/$common.js'
    ...
    (
      ...
      return [
        $column(spacing.default)(
          $node(),
          (isDesktopScreen
            ? $row(spacing.big, style({  placeContent: 'space-between', alignItems: 'center' }))
            : $row(
                spacing.big,
          ...
          switchMap(params => {
            ...
            return $Table({
              $headerRowContainer: $defaultTableRowContainer(style({ padding: '0 18px' })),
              $rowContainer: $defaultTableRowContainer(style({ padding: '0 18px', backgroundColor: pallete.background })),
              $container: $defaultTableContainer,
              $cell: $defaultTableCell(style({ padding: '0', height: '80px' })),
              scrollConfig: {
                $container: $column(style({ gap: '1px' })),
                $loader: style({
                  placeContent: 'center',
                  margin: '0 1px',
