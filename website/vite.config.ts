...
const REQUIRED_DEV_ENV = ['INDEXER_URL', 'RPC_URL', 'ORCHESTRATOR_API_KEY'] as const

export default defineConfig(({ command }) => {
  if (command === 'serve') {
    for (const key of REQUIRED_DEV_ENV) {
      if (!process.env[key]) throw new Error(`${key} is required for dev server`)
    }
  }
