...
export const $TraderMatchingRouteEditor = (config: ITraderMatchingRouteEditor) => component((
  ...
  const {
    ...
    trader, matchRuleList, collateralToken, traderMatchingRuleList
  } = config


  const rule = config.accountInfo.address ? traderMatchingRuleList.find(mr => mr.puppet === config.accountInfo.address) : undefined
    ...
    $Popover({
      ...
      $target: $ButtonSecondary({
        ...
        $container: $defaultMiniButtonSecondary(style({
          ...
          borderColor: rule && rule.expiry > unixTimestampNow() ? pallete.primary : colorAlpha(pallete.foreground, .25)
        }))
      })({
    ...
    })({}),
    {
      changeMatchRuleList: mergeArray([
        snapshot((list, params) => {
          ...
          return newList
        }, matchRuleList, combineObject({ saveDraft })),
        snapshot((list, draft) => {
          const index = list.findIndex(x =>
            x.trader === trader
          )
